...
int main(int argc, char* argv[]) {
    ...
    // Create the custom dataset (loads data_sub1.bin and data_sub2.bin)
    auto dataset = CustomDataset();
    // Create DataLoader with batch_size=1 (essential for handling pre-batched tensors)
    auto data_loader = torch::data::make_data_loader(
        std::move(dataset),
        torch::data::DataLoaderOptions(1) // Retrieves one pre-batched tensor at a time
    );

    // Load the exported AOTInductor models
    torch::inductor::AOTIModelPackageLoader compressor("exported_model/caesar_compressor.pt2");
    torch::inductor::AOTIModelPackageLoader hyper_decompressor("exported_model/caesar_hyper_decompressor.pt2"); // This is for verificaiton and is not necessary in the compression stage
    ...
    /*
    std::cout << "vbr_quantized_cdf shape: " << vbr_quantized_cdf.size() << ", " << (vbr_quantized_cdf.empty() ? 0 : vbr_quantized_cdf[0].size()) << std::endl;
    std::cout << "vbr_cdf_length shape: " << vbr_cdf_length.size() << std::endl;
    std::cout << "vbr_offset shape: " << vbr_offset.size() << std::endl;
    std::cout << "gs_quantized_cdf shape: " << gs_quantized_cdf.size() << ", " << (gs_quantized_cdf.empty() ? 0 : gs_quantized_cdf[0].size()) << std::endl;
    std::cout << "gs_cdf_length shape: " << gs_cdf_length.size() << std::endl;
    std::cout << "gs_offset shape: " << gs_offset.size() << std::endl;
    */

    
    // --- Compression Stage ---
    int batch_index = 0;
    for (auto& batch : *data_loader) { // Loop through the 2 pre-batched tensors
        std::cout << "\n--- Processing Batch " << batch_index << " ---" << std::endl;
        
        // Get the input tensor for the current batch and move to device
        torch::Tensor input_tensor = batch[0].data.to(device);
        std::cout << "Input shape: " << input_tensor.sizes() << std::endl;

        std::vector<torch::Tensor> inputs = {input_tensor};
        // Run the compressor model
        std::vector<torch::Tensor> outputs = compressor.run(inputs);
        ...
        //torch::Tensor B = outputs[4]; // Assuming B is batch size info if needed

        // Print shapes for verification
        /*
        std::cout << "Output 'q_latent' shape: " << q_latent.sizes() << std::endl;
        std::cout << "Output 'latent_indexes' shape: " << latent_indexes.sizes() << std::endl;
        std::cout << "Output 'q_hyper_latent' shape and type: " << q_hyper_latent.sizes() << ", " << q_hyper_latent.dtype() << std::endl;
        // std::cout << "Output 'q_hyper_latent' shape 0: " << q_hyper_latent.sizes()[0] << std::endl; // Might error if dimensions are 0
        std::cout << "Output 'hyper_indexes' shape: " << hyper_indexes.sizes() << std::endl;
        // std::cout << "Output B: " << B << std::endl; // Print B if necessary
        */

        std::vector<std::string> encoded_latents;
        std::vector<std::string> encoded_hyper_latents;
        
        // Loop through each item within the batch for range coding
        for (int i = 0; i < q_latent.sizes()[0]; i++){
            // Convert tensor slices to std::vector<int32_t> for the range coder
            std::vector<int32_t> latent_symbol = tensor_to_vector<int32_t>(q_latent.select(0, i).reshape(-1));
            std::vector<int32_t> latent_index = tensor_to_vector<int32_t>(latent_indexes.select(0, i).reshape(-1));
            std::vector<int32_t> hyper_symbol = tensor_to_vector<int32_t>(q_hyper_latent.select(0, i).reshape(-1));
            std::vector<int32_t> hyper_index = tensor_to_vector<int32_t>(hyper_indexes.select(0, i).reshape(-1));

            // Encode latent variables using Gaussian CDFs
            std::string latent_encoded_output = range_encoder.encode_with_indexes(
                latent_symbol,
                latent_index,
                gs_quantized_cdf,
                gs_cdf_length,
            ...
            // Encode hyper-latent variables using VBR CDFs
            std::string hyper_encoded_output = range_encoder.encode_with_indexes(
                hyper_symbol,
                hyper_index,
                vbr_quantized_cdf,
                vbr_cdf_length,
            ...
            // Store encoded bitstreams
            encoded_latents.push_back(latent_encoded_output);
            encoded_hyper_latents.push_back(hyper_encoded_output);
        }

        // Save the encoded bitstreams for the current batch
        save_encoded_streams(encoded_latents, "output/encoded_latents_batch_" + std::to_string(batch_index) + ".bin");
        save_encoded_streams(encoded_hyper_latents, "output/encoded_hyper_latents_batch_" + std::to_string(batch_index) + ".bin");
        
        // --- Verification: Load saved streams and compare ---
        std::vector<std::string> loaded_latents = load_encoded_streams("output/encoded_latents_batch_" + std::to_string(batch_index) + ".bin");
        std::vector<std::string> loaded_hyper_latents = load_encoded_streams("output/encoded_hyper_latents_batch_" + std::to_string(batch_index) + ".bin");

        if (encoded_latents == loaded_latents) {
            std::cout << "\nVerification: Loaded latents are equal to originals for batch " << batch_index << std::endl;
        } else {
            std::cout << "\nVerification Error: Loaded latents are NOT equal to originals for batch " << batch_index << std::endl;
        }

        if (encoded_hyper_latents == loaded_hyper_latents) {
            std::cout << "Verification: Loaded hyper latents are equal to originals for batch " << batch_index << std::endl;
        } else {
            std::cout << "Verification Error: Loaded hyper latents are NOT equal to originals for batch " << batch_index << std::endl;
        }

        std::cout << "loaded_hyper_latents.size(): " << loaded_hyper_latents.size() << std::endl;

        // --- Verificaiton: entropy decoding ---
        ...
        //std::vector<int32_t> hyper_index_vector = build_indexes_vector(hyper_size);
        torch::Tensor hyper_index_tensor = build_indexes_tensor(hyper_size);
        
        //std::vector<std::vector<int32_t>> decoded_hyper_latents;
        torch::Tensor decoded_hyper_latents = torch::zeros_like(q_hyper_latent);
        ...
        for (int i = 0; i < loaded_hyper_latents.size(); i++){
            ...
            //decoded_hyper_latents.push_back(hyper_decoded_output);
            ...
            // Verification checks (optional but useful for debugging)
            if (hyper_symbol != hyper_decoded_output) {
                std::cout << "Warning: hyper_symbol and hyper_decoded_output are not equal for item " << i << "." << std::endl;
            }
        }

        if (torch::equal(q_hyper_latent, decoded_hyper_latents)) {
            std::cout << "\nVerificaiton: decoded_hyper_latents are equal to q_hyper_latents" << std::endl;
        } else {
            std::cout << "\nVerificaiton Error: decoded_hyper_latents are NOT equal to q_hyper_latents" << std::endl;
        }

        std::vector<torch::Tensor> hyper_decoded_inputs = {decoded_hyper_latents.to(torch::kFloat32)};
        // Run the compressor model
        ...
        torch::Tensor decoded_latents_before_offset = torch::zeros_like(q_latent);
        ...
        for (int i = 0; i < loaded_latents.size(); i++){
            ...
            if (latent_symbol != latent_decoded_output) {
                std::cout << "Warning: latent_symbol and latent_decoded_output are not equal." << std::endl;
            }
        }

        torch::Tensor decoded_latents = decoded_latents_before_offset + mean;
        
        if (torch::equal(q_latent, decoded_latents)) {
            std::cout << "\nVerificaiton: decoded_latents are equal to q_latents" << std::endl;
        } else {
            std::cout << "\nVerificaiton Error: decoded_latents are NOT equal to q_latents" << std::endl;
        }
        
        batch_index++; // Increment batch index for the next iteration
    }
    ...
    for (int b =0; b < batch_index; i++){
        ...
        for (int i =0; i < loaded_latents.size(); i++){
            ...
            std::vector<int32_t> latent_decoded_output = range_decoder.decode_with_indexes(latent_encoded_output,
            latent_index,
            gs_quantized_cdf,
            gs_cdf_length,
            gs_offset
            ...
            std::vector<int32_t> hyper_decoded_output = range_decoder.decode_with_indexes(hyper_encoded_output,
            hyper_index,
            vbr_quantized_cdf,
            vbr_cdf_length,
            vbr_offset
    ...
    // Create a new dataset using the decoded latents (which are now the pre-batched inputs for decompression)
    auto dataset_latent = std::make_shared<PreBatchedDataset>(std::move(batches));
    
    // Check dataset size (should be 2)
    std::cout << "\nDecompression Dataset size (number of batches): " << dataset_latent->size().value() << std::endl;
    std::cout << "---" << std::endl;
